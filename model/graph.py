...
class Graph:
    ...
    def __init__(_self, root):
        _self.root = root
        #_self.dev = Potentiostat(_self.port)

    def start(_self, data):
        # data['volt_max'] !== data.volt_max
        print('data kommt als:', data)
        """
        amplitude = data['volt_max']-data['volt_min']/2.0            # Waveform peak amplitude (V) 
        offset = data['volt_max']+data['volt_min']/2.0               # Waveform offset (V) )
    ...
    def draw(_self):
        print('todo - draw graph from data', _self.root.get_values())
